...
struct MainView: View {
    ...
    @StateObject private var bookingVM = MyBookingViewModel()  // ✅ ViewModel واحد فقط
    ...
    private var myBookingSection: some View {
        VStack(alignment: .leading, spacing: 15) {
            ...
            // ✅ استخدام upcomingBooking من bookingVM
            if let booking = bookingVM.upcomingBooking,
               let room = viewModel.boardrooms.first(where: { $0.id == booking.fields.boardroom_id }) {
                RoomCardView(
                    ...
                    tag: formatDate(booking.fields.date),
